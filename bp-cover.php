...
if ( !defined( 'BPCO_PLUGIN_VERSION' ) )
	define( 'BPCO_PLUGIN_VERSION', '2.0.0' );
...
add_action( 'wp_head', 'bp_cover_scripts' );
...
function bp_cover_load_template () {
       global $bp;
	   $theme=get_option( 'bp_cover_skin' );
	      if (!empty($theme)){
		  require_once ( BPCO_PLUGIN_DIR .'templates/'. $theme . '/theme.php' );
		  }else{
		  require_once ( BPCO_PLUGIN_DIR .'templates/default/theme.php' );
         }		  
 }
add_action('bp_located_template','bp_cover_load_template');
...
function select_pic_for_cover($activity_id ) {
   ...
   $fileurl = wp_get_attachment_image_src( $attachment_id );
...
function core_get_user_email_box( $user_id ) {	
	$user_info = get_userdata( $user_id );
...
extract( $r, EXTR_SKIP ); 
 ...
 $size = @getimagesize( $filename );
         ...
         /* Check image size and shrink if too large */
         if ( $size[0] > 100 ) {
            $thumb = image_resize( $filename, 100, 100, true );
 
            // Check for thumbnail creation errors 
            if ( is_wp_error( $thumb ) ) {
               $imgresponse[0] = "0";
               $imgresponse[1] = sprintf( __( 'Upload Failed! Error was: %s', 'bp-cover' ), $thumb->get_error_message() );
               die();
            }
		...
		$attachment_id = wp_insert_attachment($attachment,$title);			
		bp_activity_update_meta( $activity_id, 'bp_cover_activity', $attachment_id );
        update_post_meta($attachment_id, 'bp_cover_thumb', $imgresponse[2]);		
}
...
add_filter('bp_core_mysteryman_src','bn_new_mysteryman' );
